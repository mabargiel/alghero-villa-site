...
The homepage SHALL render a highlights section containing a grid of cards, one per house area. Each card SHALL display a background image fetched from Sanity CMS, a Lucide icon, and an uppercase title centered over a dark gradient overlay. Each card SHALL link to the `/villa` page.
