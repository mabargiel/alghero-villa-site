...
The beach section SHALL render 3 featured hero cards (Mugoni, Le Bombarde, La Pelosa) as large full-width cards with image, name, highlight text, description, drive time, and attribute tags. Between featured cards, remaining beaches SHALL display in a responsive grid (2 columns on mobile, 3-4 on desktop). Beach data SHALL come from the TypeScript data file. Beach images SHALL come from Sanity CMS, matched by `locationKey`.

#### Scenario: Featured card for Mugoni renders
- **WHEN** the beach section loads
- **THEN** a large hero card displays for "Porto Conte / Mugoni" with "Nasza ulubiona!" highlight, full description, drive time "22 min", and attribute tags
...
- **THEN** beaches are grouped under translated sub-headers: "Plaże w Alghero", "Wybrzeże południowe", and "Plaże w okolicy"
...
The Miasteczka section SHALL display cards for Alghero, Bosa, and Castelsardo. Cards SHALL be larger than beach cards (2 per row on desktop). Each card SHALL show an image from Sanity, the town name, and a brief translated description. Hover expand and mobile bottom sheet behaviors SHALL apply.

#### Scenario: Towns render with images
- **WHEN** the Miasteczka section loads
- **THEN** three large cards display for Alghero, Bosa, and Castelsardo with images and descriptions
