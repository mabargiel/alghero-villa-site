...
Meta Pixel + Conversions API integration: the browser pixel boots under cookie consent, fires `PageView` on every route change and `ViewContent`/`InitiateCheckout`/`Lead` at the right user interactions; `/api/contact` mirrors the `Lead` event server-side via the Conversions API with matching `event_id` for deduplication; PII is SHA-256 hashed per Meta's normalization rules before transmission.
