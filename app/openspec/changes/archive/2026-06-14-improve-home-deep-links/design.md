...
Home page CTAs route to `/villa` and `/location` to give visitors deeper detail on the villa rooms / exterior areas and on the surrounding region. Today some of those links lack anchors entirely (`AreaHighlights` tiles, beaches CTA, location section), and the ones that _do_ have anchors (`/villa#salon`, `/villa#ext-garden`) land the heading flush at `y=0` where the fixed `TopNav` (~80–96px tall) covers it. From a visitor's perspective this reads as "the link didn't go to the right section."
...
- **`scroll-padding-top` also applies on the destination pages' _internal_ nav clicks** (e.g. `VillaSubNav` or `LocationHorizontalNav`) — but those already need the same offset for the same reason, so this is a feature, not a bug.
...
- **Sticky `VillaSubNav` is a _side_ nav on `xl` screens** (positioned `right-6`, vertically centered), not a top bar — so it doesn't stack on top of `TopNav` and doesn't need extra offset. Verified by reading the component.
